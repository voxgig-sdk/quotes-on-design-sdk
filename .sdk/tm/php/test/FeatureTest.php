...
// QuotesOnDesign SDK feature test
//
...
class FtCtrl extends QuotesOnDesignControl
{
...
class FtClient
{
    ...
    private QuotesOnDesignUtility $utility;

    public function __construct(QuotesOnDesignUtility $utility, array $options)
    {
    ...
    public function get_utility(): QuotesOnDesignUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (QuotesOnDesignContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public QuotesOnDesignContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new QuotesOnDesignUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (QuotesOnDesignContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = QuotesOnDesignConfig::make_feature($name);
    ...
    private static function buildUrl(QuotesOnDesignSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof QuotesOnDesignError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new QuotesOnDesignResponse($fetched);
        }
    ...
    private function fail(QuotesOnDesignContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (QuotesOnDesignContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
