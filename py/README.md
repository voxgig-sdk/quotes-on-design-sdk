...
The Python SDK for the QuotesOnDesign API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Post(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.QuotesOnDesign(None).load(
    {"id": "test01"}, None
)
