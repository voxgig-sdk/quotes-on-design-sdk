...
require_relative 'QuotesOnDesign_types'
...
class QuotesOnDesignSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = QuotesOnDesignHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = QuotesOnDesignHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, QuotesOnDesignFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    QuotesOnDesignUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = QuotesOnDesignHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = QuotesOnDesignHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = QuotesOnDesignHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue QuotesOnDesignError => err
    ...
    if fetched.is_a?(Hash)
      status = QuotesOnDesignHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = QuotesOnDesignError.new(
        "graphql_error", "QuotesOnDesignSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = QuotesOnDesignSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
