...
class QuotesOnDesignMakeRequest
{
    public static function call(QuotesOnDesignContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new QuotesOnDesignResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new QuotesOnDesignResponse($fetched);
        } else {
