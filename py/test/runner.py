# QuotesOnDesign SDK test runner
...
class QuotesOnDesignTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            QuotesOnDesignTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = QuotesOnDesignTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = QuotesOnDesignTestRunner.getenv("QUOTES_ON_DESIGN_TEST_LIVE")
        override = QuotesOnDesignTestRunner.getenv("QUOTES_ON_DESIGN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = QuotesOnDesignTestRunner.getenv(key)
        ...
        explain = QuotesOnDesignTestRunner.getenv("QUOTES_ON_DESIGN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if QuotesOnDesignTestRunner._test_control is not None:
            return QuotesOnDesignTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                QuotesOnDesignTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            QuotesOnDesignTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = QuotesOnDesignTestRunner.load_test_control()
...
def load_env_local():
    QuotesOnDesignTestRunner.load_env_local()


def env_override(m):
    return QuotesOnDesignTestRunner.env_override(m)


def entity_data(v):
    return QuotesOnDesignTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return QuotesOnDesignTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return QuotesOnDesignTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return QuotesOnDesignTestRunner.load_test_control()


def live_client_options():
    return QuotesOnDesignTestRunner.live_client_options()


def live_delay_ms():
    return QuotesOnDesignTestRunner.live_delay_ms()
