# QuotesOnDesign PHP SDK

The PHP SDK for the QuotesOnDesign API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Post(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->QuotesOnDesign(null)->load(
    ["id" => "test01"], null
);
...
```
php/
├── quotesondesign_sdk.php          -- Main SDK class
...
The main class (`quotesondesign_sdk.php`) exports the SDK class
