...
The Lua SDK for the QuotesOnDesign API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Post(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:QuotesOnDesign(nil):load(
  { id = "test01" }, nil
)
