...
//     (\`new QuotesOnDesignSDK(...)\` / \`QuotesOnDesignSDK.test(...)\` become
//     \`QuotesOnDesignSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'QuotesOnDesignSDK'
...
// rewrite real client construction (\`new QuotesOnDesignSDK(...)\` and any
// \`QuotesOnDesignSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+QuotesOnDesignSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/QuotesOnDesignSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
