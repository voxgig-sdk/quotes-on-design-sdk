# quotes-on-design-mcp

[MCP](https://modelcontextprotocol.io) server exposing the QuotesOnDesign SDK as
two agent tools — `quotes-on-design_list` and `quotes-on-design_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quotes-on-design-mcp)
make build

# 2. Provide credentials via the environment
export QUOTES_ON_DESIGN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user quotes-on-design \
  -- /absolute/path/to/quotes-on-design-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./quotes-on-design-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// quotes-on-design_list: first page of records
{ "entity": "post" }
{ "entity": "post", "query": { } }

// quotes-on-design_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quotes-on-design-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user quotes-on-design \
     -- "$PWD"/dist/*/quotes-on-design-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `quotes-on-design_list` and `quotes-on-design_load` tools now appear
   in new sessions. Ask the agent to *"list post using quotes-on-design"*
   and it calls `quotes-on-design_list` with `{"entity":"post"}`.
...
```sh
export QUOTES_ON_DESIGN_APIKEY=sk_live_xxx            # API key
export QUOTES_ON_DESIGN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `quotes-on-design_list` tool
...
### Call the `quotes-on-design_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `quotes-on-design_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `quotes-on-design_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `QUOTES_ON_DESIGN_APIKEY` | API key sent with every request. |
| `QUOTES_ON_DESIGN_BASE` | Optional override of the API base URL. |
...
```sh
./quotes-on-design-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"quotes-on-design_load","arguments":{"entity":"post","query":{"id":1}}}}'
```
