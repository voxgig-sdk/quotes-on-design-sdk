...
//    only - a typed QuotesOnDesignContext there would make every ctx
...
final class QuotesOnDesignOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new QuotesOnDesignContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new QuotesOnDesignSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new QuotesOnDesignResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new QuotesOnDesignError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new QuotesOnDesignResponse($clean['response']);
    ...
    public static function err_from_map($m): ?QuotesOnDesignError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new QuotesOnDesignError(is_string($code) ? $code : '', $msg);
    }
