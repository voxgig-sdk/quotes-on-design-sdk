...
func TestPostEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set QUOTESONDESIGN_TEST_POST_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func postBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("QUOTESONDESIGN_TEST_POST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["QUOTESONDESIGN_TEST_POST_ENTID"])
