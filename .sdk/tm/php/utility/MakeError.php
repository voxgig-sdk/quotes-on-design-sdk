...
class QuotesOnDesignMakeError
{
    public static function call(?QuotesOnDesignContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new QuotesOnDesignContext([], null);
        }
        $op = $ctx->op ?? new QuotesOnDesignOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new QuotesOnDesignResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof QuotesOnDesignError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new QuotesOnDesignError('', $msg, $ctx);
        ...
        if ($err instanceof QuotesOnDesignError) {
