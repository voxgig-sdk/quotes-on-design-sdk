# quotes-on-design-cli

boru-driven command-line client **and** interactive REPL for the QuotesOnDesign
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quotes-on-design-cli)
make build

# 2. See usage (words, entities, env vars)
./quotes-on-design-cli --help

# 3. Provide credentials once, via the environment
export QUOTES_ON_DESIGN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./quotes-on-design-cli list post
./quotes-on-design-cli load 1 post            # {id:1} shorthand
./quotes-on-design-cli load '{id:1}' post       # explicit match map

# 5. Override the API base URL for a single call
QUOTES_ON_DESIGN_BASE=https://api.example.com ./quotes-on-design-cli list post

# 6. No arguments -> interactive REPL
./quotes-on-design-cli
quotes-on-design> list post
quotes-on-design> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quotes-on-design-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/quotes-on-design-cli list post
   ```
...
```sh
./quotes-on-design-cli load 1 post          # scalar shorthand for {id:1}
./quotes-on-design-cli load '{id:1}' post     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `quotes-on-design>`). Each line is
evaluated as its own boru expression:

```text
$ ./quotes-on-design-cli
quotes-on-design> list post
quotes-on-design> /help
quotes-on-design> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/quotes-on-design-cli`. |
...
(`./quotes-on-design-cli <expr>`) and interactively (the REPL), and expressions compose the
