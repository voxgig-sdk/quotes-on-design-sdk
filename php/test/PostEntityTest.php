...
class PostEntityTest extends TestCase
{
    ...
    {
        $testsdk = QuotesOnDesignSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = QuotesOnDesignSDK::test($seed, null);
        $seen = iterator_to_array($base->Post(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = QuotesOnDesignConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = QuotesOnDesignSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set QUOTES_ON_DESIGN_TEST_POST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function post_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("QUOTES_ON_DESIGN_TEST_POST_ENTID");
    ...
    if ($env["QUOTES_ON_DESIGN_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new QuotesOnDesignSDK(Helpers::to_map($merged_opts));
    }
