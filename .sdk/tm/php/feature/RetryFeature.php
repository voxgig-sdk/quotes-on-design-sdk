...
// QuotesOnDesign SDK retry feature
...
class QuotesOnDesignRetryFeature extends QuotesOnDesignBaseFeature
{
    ...
    public function init(QuotesOnDesignContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (QuotesOnDesignContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(QuotesOnDesignContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(QuotesOnDesignContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
