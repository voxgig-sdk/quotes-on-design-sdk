...
// QuotesOnDesign SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class QuotesOnDesignTestFeature extends QuotesOnDesignBaseFeature
{
    ...
    public function init(QuotesOnDesignContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (QuotesOnDesignContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = QuotesOnDesignParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (QuotesOnDesignContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(QuotesOnDesignContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = QuotesOnDesignParam::call($ctx, $k);
