...
import (
	...
	omni "github.com/voxgig-sdk/quotes-on-design-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.QuotesOnDesignError); is {
				out := map[string]any{
					"name":    "QuotesOnDesignError",
...
func omniCtx(arg any, client *sdk.QuotesOnDesignSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.QuotesOnDesignSDK); is {
			client = live
