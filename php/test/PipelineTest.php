...
// QuotesOnDesign SDK pipeline test
//
...
{
    private static function utility(): QuotesOnDesignUtility
    {
        return new QuotesOnDesignUtility();
    }
    ...
    private static function ctx(array $over = []): QuotesOnDesignContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new QuotesOnDesignContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new QuotesOnDesignOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof QuotesOnDesignError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new QuotesOnDesignOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = QuotesOnDesignMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new QuotesOnDesignOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = QuotesOnDesignMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new QuotesOnDesignOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = QuotesOnDesignMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new QuotesOnDesignSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = QuotesOnDesignMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new QuotesOnDesignResponse([]),
            'result' => new QuotesOnDesignResult([]),
        ]);
        [, $err] = QuotesOnDesignMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new QuotesOnDesignSpec([]),
            'response' => null,
            'result' => new QuotesOnDesignResult([]),
        ]);
        [, $err] = QuotesOnDesignMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new QuotesOnDesignSpec(['step' => 's']),
            'response' => new QuotesOnDesignResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new QuotesOnDesignResult([]),
        ]);
        [, $err] = QuotesOnDesignMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new QuotesOnDesignSpec(['step' => 's']),
            'response' => new QuotesOnDesignResponse(self::resp(200, ['v' => 1])),
            'result' => new QuotesOnDesignResult([]),
        ]);
        [, $err] = QuotesOnDesignMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new QuotesOnDesignSpec(['step' => 's']),
            'response' => new QuotesOnDesignResponse(self::resp(200, ['v' => 2])),
            'result' => new QuotesOnDesignResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        QuotesOnDesignMakeResponse::call($ctx);
    ...
    {
        $preset = new QuotesOnDesignResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new QuotesOnDesignSpec([]),
            'response' => new QuotesOnDesignResponse([]),
            'result' => new QuotesOnDesignResult([]),
        ...
        [$got, $err] = QuotesOnDesignMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new QuotesOnDesignResult([])]);
        [, $err] = QuotesOnDesignMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new QuotesOnDesignSpec([]), 'result' => null]);
        [, $err] = QuotesOnDesignMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new QuotesOnDesignSpec(['step' => 's']),
            'result' => new QuotesOnDesignResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new QuotesOnDesignOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = QuotesOnDesignMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new QuotesOnDesignSpec(['step' => 's']),
            'result' => new QuotesOnDesignResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new QuotesOnDesignOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = QuotesOnDesignMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new QuotesOnDesignResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new QuotesOnDesignSpec([]),
            'result' => new QuotesOnDesignResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = QuotesOnDesignMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = QuotesOnDesignMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new QuotesOnDesignError('boom', 'boom');
        $utility->fetcher = function (QuotesOnDesignContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new QuotesOnDesignSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = QuotesOnDesignMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (QuotesOnDesignContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = QuotesOnDesignMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(QuotesOnDesignResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        QuotesOnDesignMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (QuotesOnDesignContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new QuotesOnDesignSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = QuotesOnDesignMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new QuotesOnDesignResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new QuotesOnDesignSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = QuotesOnDesignMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = QuotesOnDesignMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = QuotesOnDesignMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new QuotesOnDesignResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, QuotesOnDesignDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new QuotesOnDesignResult(['ok' => false])]);
        $this->expectException(QuotesOnDesignError::class);
        QuotesOnDesignDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new QuotesOnDesignResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', QuotesOnDesignMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        QuotesOnDesignMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new QuotesOnDesignBaseFeature();
        $b = new QuotesOnDesignBaseFeature();
        QuotesOnDesignFeatureAdd::call($ctx, $a);
        QuotesOnDesignFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): QuotesOnDesignBaseFeature
    {
        $f = new QuotesOnDesignBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        QuotesOnDesignFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): QuotesOnDesignContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new QuotesOnDesignSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = QuotesOnDesignPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        QuotesOnDesignPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new QuotesOnDesignResponse(['headers' => null]),
            'result' => new QuotesOnDesignResult([]),
        ]);
        QuotesOnDesignResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        QuotesOnDesignResultBody::call($ctx);
