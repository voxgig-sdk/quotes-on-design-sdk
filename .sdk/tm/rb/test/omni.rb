...
module QuotesOnDesignOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if QuotesOnDesignOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = QuotesOnDesignOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| QuotesOnDesignOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || QuotesOnDesignOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = QuotesOnDesignSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = QuotesOnDesignError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { QuotesOnDesignOmni.errify(err) }
