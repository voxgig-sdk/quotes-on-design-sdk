...
module QuotesOnDesignUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = QuotesOnDesignResponse.new({})
    ...
    if fetch_err
      ...
      response = QuotesOnDesignResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = QuotesOnDesignResponse.new(fetched)
    else
